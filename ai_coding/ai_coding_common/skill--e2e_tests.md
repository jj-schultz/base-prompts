...
- **Minimize test methods**: 
  - There is significant overhead with running each test method (browser setup, authentication, etc.). 
  - Prefer fewer, longer test methods that verify multiple related behaviors over many small single-assertion tests. 
  - A single test method that validates an entire user flow is better than five separate methods testing each step.
  - when thinking of writing a new test, you **must** first look to see if there's an existing test you can re-use

# Step 1: Understand the Features to Test

Review the feature description provided with this skill. Identify:
