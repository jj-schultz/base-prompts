...
5. Decide what to fix next:
   - Review the 'goal.md' file if it exists.  This will contain guidance about what the changes are trying to do.  Use this information if it exists to know if the tests are broken or the code is broken
...
8. Loop back to step 2 and repeat until all of the tests in`./developer-tools/scripts/by_tests_all.sh` succeed.
...
If you think it will speed up the fixing of the tests to run a subset of the tests after step 7 (prior to fully re-running by_tests_all.sh), you can do so by utilizing by_tests_all.sh's "--tests=" parameter
