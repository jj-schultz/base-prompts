The text after the `$REGRESSION_FIX_AND_REVIEW` skill is a request to repeatedly review the current uncommitted changes with a regression-focused review, fix only the review findings that matter most, and stop once no `HIGH PRIORITY` or `MUST FIX` issues remain.
...
4. Fetch and review the current uncommitted changes using the same workflow and standards as `$REGRESSION_CHECK`:
   ...
   - apply the regression-review emphasis from [skill--regression_check.md](skill--regression_check.md)
5. Store the current loop's review output in the active `todo.md` file under this heading:
   - `## REGRESSION_FIX_AND_REVIEW Review`
6. Before appending the new review output, remove the entire previous `## REGRESSION_FIX_AND_REVIEW Review` section if it exists so the current review fully overwrites the prior loop's review results.
7. Inside `## REGRESSION_FIX_AND_REVIEW Review`, use the exact review output format required by [instructions--code_review.md](instructions--code_review.md).
8. After the review is written, inspect only the current `## REGRESSION_FIX_AND_REVIEW Review` section.
