...
Use the following principles as an internal checklist.  Before you signal you are finished, verify you've followed all requirements on this internal checklist
...
### 5. Respect the input document's milestones and phase name

- If the input document specifies milestones or phases, you must use these names for task groups
- If you choose to create more milestones or phases than are described in the document (totally valid), use the document's phase naming style as inspiration
...
When outputting tasks for Asana import, you **must** use the CSV format defined in [example--asana_task_export.csv](example--asana_task_export.csv)
...
- Generate CSV using a real CSV writer (RFC 4180); never hand-build rows with string joins
- Ensure any field containing commas, quotes, or newlines is correctly escaped/quoted (quoting all string fields is preferred)
- Validate every output row has exactly the same column count as the header before finalizing the file
