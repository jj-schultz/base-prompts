# High Level TODOs you **must** exectute every time the claude-todo.md list is run
1. when you complete an item in claude-todo.md's list, do not delete it- rather indicate it is complete with a checkmark emoji). 
2. When finished with all of the TODO items in claude-todo.md, **always** append a section to claude-todo.md that lists the files touched and a summary (including line number range) of the changes to the file. Do NOT write the summary in the conversation output - it must be appended to claude-todo.md itself
3. **never** attempt to run tests and do not announce that you are not running tests
4. When you are finished updating this file with the summary of changes and performing any/all of your validations, execute the script `./.claude/support/notify_done.sh`
...
- **never** execute `python manage.py`.  **never** run a database migration.  These are to be manual operations 
